...
export class AccountsPage {
  ...
  async openAccountDetailsPage() {
    ...
    if (accountNumber) {
      const accountNumberLink = this.page.getByRole("link", {
        name: accountNumber,
      });
    ...
    for (const [rowIndex, row] of rows.entries()) {
      ...
      for (const [colIndex, cell] of cells.entries()) {
        ...
        if (cellText?.trim() && cellText !== "Total") {
          ...
          expect(hasNumber, `Cell at row ${rowIndex + 1}, column ${colIndex + 1} should contain a number`).toBeTruthy();
        }
      }
    }
  }
  ...
  async findAccountNumber(accountId: string) {
    let accountNumber = "";
    const allRows = await this.getAllRowsInTable(this.accountTable);
  ...
  async assertAccountNumberIsPresent(accountId: string) {
    expect(this.findAccountNumber(accountId)).toBeTruthy();
  }
}
